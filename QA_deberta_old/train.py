"""
Fine-tune DebertaQAModel on CUAD — self-contained version.

Reads preprocessed CUAD from QA_deberta/data/{train,val}.json.
(Run download_cuad.py first to create these.)

Usage (from NLP-Project/ root):
    python -m QA_deberta.train --epochs 5 --batch_size 8 --lr 2e-5

NOTE: DeBERTa-v3-base is ~184M params. If you hit OOM on 8GB VRAM,
drop batch_size to 4. Gradient checkpointing is already enabled.
"""
...
from QA_deberta_old.loss import compute_loss, HAS_ANSWER_LOSS_WEIGHT
...
class CuadDatasetDeberta(Dataset):
    """
    Wraps JSON format produced by download_cuad.py:
    [{"question", "context", "answer_start", "answer_text", "has_answer"}, ...]

    Unlike the BERT version, we build context_mask manually because
    ...
    def __getitem__(self, idx):
        ...
        # Build context_mask: True for tokens between the two [SEP]s.
        # Format: [CLS] question [SEP] context [SEP] [PAD]...
        ...
        # Compute token-level answer positions
        start_pos = 0  # default: [CLS] = no-answer
        end_pos   = 0
        ...
        if has_ans and "answer_start" in ex and ex["answer_start"] >= 0:
            ...
            for i, (tok_s, tok_e) in enumerate(offset_mapping.tolist()):
                if context_mask[i]:  # only search inside context
...
def quick_eval(model, loader, device, tokenizer):
    ...
    for batch in loader:
        ...
        # Disallow position 0 — we trained on positives only
        s_logits_no_cls = s_logits.clone()
        e_logits_no_cls = e_logits.clone()
        s_logits_no_cls[:, 0] = float("-inf")
        e_logits_no_cls[:, 0] = float("-inf")
        pred_starts = s_logits_no_cls.argmax(dim=-1).cpu()
        pred_ends   = e_logits_no_cls.argmax(dim=-1).cpu()
...
def train(args):
    ...
    print(f"[DeBERTa-QA] device={device}")
    ...
    if not os.path.exists(train_f):
        raise FileNotFoundError(
            f"CUAD train file not found: {train_f}\n"
    ...
    log_path = os.path.join(args.output_dir, "deberta_train_log.json")
    log      = []
    best_f1  = 0.0

    print(f"[DeBERTa-QA] {len(train_ds)} train examples, {args.epochs} epochs")

    for epoch in range(1, args.epochs + 1):
        model.train()

        if epoch == 3:
            print("[DeBERTa-QA] Unfreezing top 4 encoder layers...")
            model.unfreeze_top_layers(keep_frozen=max(args.freeze_layers - 4, 0))
            deberta_params = [p for p in model.deberta.parameters() if p.requires_grad]
        ...
        if val_loader:
            ...
            if val_metrics["f1"] > best_f1:
                ...
                }, os.path.join(args.output_dir, "deberta_best.pt"))
    ...
    torch.save({"epoch": args.epochs, "model_state": model.state_dict()},
               os.path.join(args.output_dir, "deberta_final.pt"))
    print(f"[DeBERTa-QA] Done. Best val F1={best_f1:.4f}")
