"""
Comprehensive evaluation of the fine-tuned DeBERTa QA model on CUAD val set.
Vectorized span search — runs ~50-100x faster than the naive loop version.
...
Usage (from NLP-Project/ root):
    ...
    python -m QA_deberta.full_eval --n_examples 500
...
def predict_one(model, tokenizer, question: str, context: str, device,
    ...
    s_logits = s_logits[0]   # (L,)
    e_logits = e_logits[0]   # (L,)
    L = s_logits.size(0)

    # Vectorized span scoring: score[i,j] = s_logits[i] + e_logits[j]
    # Shape: (L, L)
    span_scores = s_logits.unsqueeze(1) + e_logits.unsqueeze(0)

    # Mask invalid spans: end < start OR end - start >= max_answer_len
    ...
    # Disallow [CLS] ([0,0]) as a prediction — training was positives-only,
    # so position 0 should never be the answer.
    span_scores[0, :] = float("-inf")
    span_scores[:, 0] = float("-inf")
    # The forward() in model.py already masks non-context positions to -1e9 in
    # s_logits and e_logits, so we don't need to re-mask here.

    # Single argmax over (L*L) flattened
...
def run_eval(args):
    ...
    if unanswerable_n > 0:
        ...
        print(f"    F1  = {unanswerable_f1/unanswerable_n:.4f}   (1.0 = correctly said 'no answer')")
        print(f"    EM  = {unanswerable_em/unanswerable_n:.4f}")

    print("\n" + "─" * 70)
    print("HASANSWER CLASSIFIER (pred span = 0 means 'no answer')")
    print("─" * 70)
    print(f"  True Positives  (gold+ pred+): {ha_tp}")
    print(f"  False Negatives (gold+ pred-): {ha_fn}  <- missed real answers")
    print(f"  False Positives (gold- pred+): {ha_fp}  <- hallucinated answers")
    print(f"  True Negatives  (gold- pred-): {ha_tn}")
    if (ha_tp + ha_fp) > 0:
    ...
    out_path = os.path.join(os.path.dirname(args.ckpt), "full_eval_report.json")
    report = {
        "n_total":      n_total,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt",       type=str, default="QA_deberta/checkpoints/deberta_best.pt")
