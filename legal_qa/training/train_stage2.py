"""
Stage 2 Training Script: Long Document QA.
"""
...
def get_args():
    parser = argparse.ArgumentParser(description="Train Stage 2 Long Document QA")
...
def main():
    ...
    best_stage1_checkpoint = os.path.join(config["checkpoint_dir"], "qa_stage1_best.pt")
    best_checkpoint_path = os.path.join(config["checkpoint_dir"], "qa_stage2_best.pt")
    emergency_checkpoint_path = os.path.join(config["checkpoint_dir"], "qa_stage2_emergency.pt")
    latest_checkpoint_path = os.path.join(config["checkpoint_dir"], "qa_stage2_latest.pt")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    logger.info(f"Using device: {device}")

    # ---- Data loaders ----
    logger.info("Initializing Stage 2 Datasets...")
    max_ex = 50 if args.dry_run else None

    train_dataset = LongDocQADataset(split="train", max_quality=max_ex, max_qasper=max_ex)
    val_dataset = LongDocQADataset(split="validation", max_quality=max_ex, max_qasper=max_ex)
    ...
    if os.path.exists(best_stage1_checkpoint):
        logger.info(f"Loading Stage 1 best checkpoint: {best_stage1_checkpoint}")
        ckpt_1 = torch.load(best_stage1_checkpoint, map_location="cpu")
        model.load_state_dict(ckpt_1["model_state_dict"])
    else:
        logger.warning(f"Could not find Stage 1 checkpoint at {best_stage1_checkpoint}. Starting from scratch!")
        
    model.to(device)

    # ---- Optimizer & Scheduler ----
    optimizer = get_discriminative_optimizer(model, stage=2)
    ...
    if args.resume and os.path.exists(latest_checkpoint_path):
        logger.info("Resuming from latest Stage 2 checkpoint...")
    ...
    try:
        for epoch in range(start_epoch, config["num_epochs"] + 1):
            ...
            # track_chunk_retrieval=True calculates the separate chunk tracking accuracy metric for Stage 2
            val_metrics = trainer.validate(track_chunk_retrieval=True)

            trainer.log_epoch(epoch, train_metrics, val_metrics)
            logger.info(f"Epoch {epoch} | Train Loss: {train_metrics['train_loss_total']:.4f} | Val F1: {val_metrics['val_f1']:.4f} | Val Chunk Rank: {val_metrics.get('val_chunk_retrieval_acc', 0):.4f}")
